...
async def startup_event():
    load_yolo_model()
    print(f"🚀 Startup complete, model ready at port {PORT}")
...
async def detect(file: UploadFile = File(...)):
    ...
    try:
        ...
        img_no_bg_bytes = remove(contents)  # rembg with bytes
        img_no_bg = Image.open(io.BytesIO(img_no_bg_bytes)).convert("RGB")
